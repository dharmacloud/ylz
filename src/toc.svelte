<script>
import { styledNumber,debounce, usePtk} from "ptk";
...
{#if !$loadingfolio}
<Juan {closePopup}/>
<input type="number" bind:value={folio} name="folio" on:input={debounce(setFolio,2000)} style="width:3em" />  {$maxfolio}折
